...
\begin{figure}[H]
  ...
  \caption{Log in on the Web Application}
  ...
  \begin{tabular}{rp{14cm}}
    ...
    \begin{tabular}{@{}p{14cm}@{}}
        \begin{enumerate}[leftmargin=*]
            \item The client have to click on the button of sing up of the web application
            [\ref{fig:LoginWebApp}]
...
\begin{table}[H]
  ...
  \begin{tabular}{rp{14cm}}
    ...
    \begin{tabular}{@{}p{14cm}@{}}
         \begin{enumerate}
             \item The client have to be registered on TrackMe
    ...
    \begin{tabular}{@{}p{14cm}@{}}
        \begin{enumerate}[leftmargin=*]
            \item Press the log in button in the web application [\ref{fig:LoginWebApp}]
            \item Complete the username and password sections of the log in window
            \item After clicking on the log in button, if it is correct the username and password will access to your account and the system will redirect to the main window [\ref{fig:PrincipalPageWebApp}]
        \end{enumerate} 
    \end{tabular} \\ 
    \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{rp{14cm}}
    ...
    \begin{tabular}{@{}p{14cm}@{}}
         \begin{enumerate}
             \item The user have to be registered on TrackMe
    ...
    \begin{tabular}{@{}p{14cm}@{}}
        \begin{enumerate}[leftmargin=*]
            \item Press the log in button in the application
            \item Complete the username and password sections of the log in window
            \item After clicking on the log in button, if it is correct the username and password will access to your account and the system will redirect to the main window
        \end{enumerate} 
    \end{tabular} \\ 
    \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{rp{14cm}}
    ...
    ID & \uselabel{use:login}\\
    \hline
    Name & Search for an individual data\\
    \hline
    Actor & Client\\
    \hline
    ...
    \begin{tabular}{@{}p{14cm}@{}}
        \begin{enumerate}[leftmargin=*]
            ...
            [\ref{fig:PrincipalPageWebApp}]
            \item Enter the "codice fiscale" in the search area that appears on the new page (in the area where it is requested) [\ref{fig:SearchIndividualWebApp}]
            \item The system will redirect you to one page where you can view all that person's information (if you have the necessary permissions) or you can do another search
           [\ref{fig:ResultIndividualSearch1}, \ref{fig:ResultIndividualSearch2}, \ref{fig:ResultIndividualSearch3}]
        \end{enumerate} 
    \end{tabular} \\ 
    \hline
    ...
    \begin{tabular}{@{}p{14cm}@{}}
         \begin{enumerate} [leftmargin=*]
             ...
             \item The client does not have the permissions to view that person's data
         \end{enumerate}  \\
    \end{tabular}\\
...
\begin{table}[H]
  ...
  \begin{tabular}{rp{14cm}}
    ...
    \begin{tabular}{@{}p{14cm}@{}}
        \begin{enumerate}[leftmargin=*]
            \item Click on the group data search button
            \item
            \item 
        \end{enumerate} 
    \end{tabular} \\ 
...
\begin{table}[H]
  ...
  \begin{tabular}{ccp{14cm}}
    ...
    \reqlabel{req:recollection-data-manual-functioning} &
    \goalref{gl:how-recollect} \goalref{gl:intervals} & \begin{tabular}{@{}p{14cm}@{}}
...
\begin{table}[H]
  ...
  \begin{tabular}{ccp{14cm}}
    ...
    \goalref{gl:registration} & \begin{tabular}{@{}p{14cm}@{}}
    When a message with a correct format reach the interface \siref{si:api-login} with an existing pair of username and password, the system shall replay with a token that will identify the client in the next api calls. The token have a validity of \comment{3 days}.
    \end{tabular} \\
    \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{ccp{14cm}}
    ...
    When a parameter sent by an user's application arrives at the server and is below a defined threshold and the user is sign up in AmbulanceSOS, the system shall rise an alarm to the Emergency System using interface \siref{si:emergency-alarm} within 5 seconds.
    \end{tabular} \\
    \hline
...
\begin{table}[H]
  ...
  \begin{tabular}{cp{14cm}}
    ...
    The system shall be able to handle at least \comment{10000 active users}.
    \end{tabular} \\
    \hline
    \perflabel{perf:number-of-clients} & \begin{tabular}{@{}p{14cm}@{}}
    The system shall be able to handle at least \comment{100 active clients}.
    \end{tabular} \\
    \hline
...
\begin{itemize}
    \item The application will be installed in the internal memory of the terminal and will store all the information in it.
    \item The application will request and need the acceptance of some permissions for access to the terminal, if not accepted the application can not work or show a proper operation.
    \item The app will be kept in the background with a minimum consumption for the use of notifications if necessary.
\end{itemize}
...
In addition to what has been said for the mobile phone, it is also necessary that the user has:
 \begin{itemize}
        ...
        \item Possibility of receiving calls
 \end{itemize}
...
The system will operate continuously and must be operational at all times, in the event of a system crash by any internal or external problem services will be redirected to secondary servers that will be present in support format for abnormal situations to perform the continuous running.

\subsubsection{Security}
For security, the HTTPS protocol will be used, which applies SSL/TLS cryptography that will allow the passage between the server and the different services in a secure way, not allowing attacks or loss of information from TrackMe clients and users.

\subsubsection{Maintainability}
The maintenance of TrackMe services will be performed by the system administrators, this maintenance will be designed to be as simple as possible using general programming formats as well as a worked documentation in which the system is explained as accurately as possible to save any doubt to the administrators about the operation of the various services.

\subsubsection{Portability}
The portability degree depends on each system's component. The mobile application shall be portable between Android platforms that fullfills the requirements stated in section \ref{sec:constraints}. The server component shall be portable between Linux platforms that fullfills the requirements stated in section \ref{sec:constraints}. The online dashboard  shall work in any browser that fullfills the rules stated in section \ref{sec:constraints}
