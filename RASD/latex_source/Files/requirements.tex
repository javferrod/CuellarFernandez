...
\begin{figure}[H]
  ...
  \caption{\textit{Log in and sing up on the Web Application}}
...
\begin{table}[H]
  ...
  \end{tabular}
  \end{adjustbox}
  \caption{Log in use case (Client)}
  ...
  \begin{tabular}{rp{14cm}}
    ...
    \begin{tabular}{@{}p{14cm}@{}}
        \begin{enumerate}[leftmargin=*]
            ...
            \item After pressing the search button the system will show the information of the users (anonymously) who meet the criteria given. \comment{Ref. Mockup}
        \end{enumerate} 
    \end{tabular} \\ 
    \hline
...
\comment{Añadir introducción a los diagramas de secuencia}
...
\comment{Añadir información sobre el diagrama de secuencia de trackeo}
...
\begin{table}[H]
  ...
  \begin{tabular}{ccp{14cm}}
    ...
    When the client introduces a query from the dashboard page (Figure \ref{fig:GroupSearch})\comment{REVISAR mockup} and the number of entries that fullfil the query are equal or more than 1000, the system shall show the data in page \comment{ref al mockup}.
    \end{tabular} \\
    \hline
    
    \reqlabel{req:client-query-few} &
    \goalref{gl:query} \goalref{gl:privacy}& \begin{tabular}{@{}p{14cm}@{}}
    When the client introduces a query from the dashboard page (Figure \ref{fig:GroupSearch})\comment{REVISAR mockup} and the number of entries that fullfil the query are less than 1000, the system shall warn the client about the impossibility to show the results in page \comment{ref al mockup}.
    \end{tabular} \\
    \hline
    ...
    \goalref{gl:individuals} & \begin{tabular}{@{}p{14cm}@{}}
    When the client introduces a codice fiscale from \comment{pagina de busqueda de individuos} (Figure \ref{fig:SearchIndividualWebApp}), the user exists and the client have already obtained the permission of the user, the system shall return the data associated to the individual.
    \end{tabular} \\
    \hline
    
    \reqlabel{req:client-codice-permission} &
    \goalref{gl:individuals} & \begin{tabular}{@{}p{14cm}@{}}
    When the client introduces a codice fiscale from \comment{pagina de busqueda de individuos} (Figure \ref{fig:SearchIndividualWebApp}), the user exists and the client do not have the permission of the user, the system shall prompt the client to ask permission to the user.
    \end{tabular} \\
    \hline
    
    \reqlabel{req:client-codice-noexist} &
    \goalref{gl:individuals} & \begin{tabular}{@{}p{14cm}@{}}
    When the client introduces a codice fiscale from \comment{pagina de busqueda de individuos} (Figure \ref{fig:SearchIndividualWebApp}), and the user do not exists, the system shall prompt the client to ask permission to the user\footnote{This is done to prevent the clients to poll if a codice fiscale is in the system or not}.
    \end{tabular} \\
    \hline
